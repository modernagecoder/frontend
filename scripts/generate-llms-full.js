...
const COURSE_DESC_TRIM = 400;
const PER_BLOG_FULL_TRIM = 4500;
const TOTAL_SOFT_CAP = 100 * 1024;
...
function buildCourseCatalog() {
    ...
    const courses = files
        .map(f => { try { return loadJson(path.join(COURSE_DATA, f)); } catch { return null; } })
        .filter(c => c && c.meta && c.meta.slug)
        .map(c => c.meta);

    const buckets = { Kids: [], Teens: [], College: [], Mathematics: [], Girls: [], Other: [] };
    ...
    out += `Every Modern Age Coders course, grouped by audience.\n\n`;
...
function buildBlogSections() {
    ...
    const blogs = files
        .map(f => { try { return loadJson(path.join(BLOG_DATA, f)); } catch { return null; } })
    ...
    full += `The ${featured.length} highest-priority articles, embedded in full.\n\n`;
...
- **Locations:** Online across India (Mumbai, Kolkata, Bengaluru, Delhi, Hyderabad, Pune, Chennai, Ahmedabad, Indore) and international (USD, EUR, GBP supported).
...
function generate() {
    ...
    const body = [header, facts, courses, blogs, contact].join('\n');
    ...
    if (sizeBytes > TOTAL_SOFT_CAP) {
        console.warn(`⚠️  Over ${(TOTAL_SOFT_CAP / 1024).toFixed(0)} KB soft cap.`);
    }
}
