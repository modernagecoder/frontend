/**
 * College Students Blog JSON Validation Script
 * Validates blog files against the design document requirements (Correctness Properties)
 */

const fs = require('fs');
const path = require('path');

// College Students category blog files to validate
const blogFiles = [
  'best-programming-languages-college-2025.json',
  'college-students-build-real-world-projects.json',
  'project-based-learning-coding.json',
  'best-free-paid-coding-courses-students.json',
  'coding-mathematics-engineering-science.json',
  'competitive-coding-vs-development-skills.json'
];
...
function validateBlog(filename) {
  ...
  try {
    ...
    validationResult.errors.push(`Property 9 FAIL - Invalid JSON or file not found: ${error.message}`);
  ...
  if (!hasRequiredKeys) {
    validationResult.errors.push(`Property 9 FAIL - Missing required keys. Found: ${Object.keys(blogData).join(', ')}`);
  }
  ...
  if (!headingHierarchyValid) {
    validationResult.warnings.push('Property 2 WARNING - H3 heading appears before any H2 heading');
  }

  // Property 4: Internal links to /courses/ (or /courses without trailing slash)
...
console.log('BLOG JSON VALIDATION - College Students Category');
...
// Final summary
console.log('\n' + '='.repeat(70));
console.log('VALIDATION SUMMARY - College Students Category');
console.log('='.repeat(70));
...
console.log('\n' + '='.repeat(70));
console.log(allPassed ? '✅ ALL COLLEGE STUDENTS BLOGS PASSED VALIDATION!' : '❌ SOME VALIDATIONS FAILED');
console.log('='.repeat(70));
