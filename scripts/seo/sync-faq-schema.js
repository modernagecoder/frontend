...
files.forEach((rel) => {
  ...
  // three FAQ markups in use on the site: <details><summary>Q</summary><div class="faq-a">A</div>,
  // <details><summary>Q</summary><p>A</p>, and <div class="faq-item"><div class="faq-question"><span>Q</span>…<div class="faq-answer"><p>A</p>
  const vis = [];
  [...html.matchAll(/<details[^>]*>\s*<summary>([\s\S]*?)<\/summary>\s*(?:<div class="faq-a">([\s\S]*?)<\/div>|<p>([\s\S]*?)<\/p>)/g)].forEach((m) => vis.push([norm(dec(m[1])), norm(dec(m[2] || m[3]))]));
