...
// Market for the lead form and the locale tags. A cg module without a market
// object is a Netherlands page, the cluster this renderer was written for, so
// its output is byte-for-byte what it was.
const NL_MARKET = { iso: 'NL', dial: '+31', name: 'Netherlands', locale: 'en_NL', geoRegion: 'NL', phoneLabel: 'Dutch mobile number', phonePlaceholder: '6 XXXX XXXX', gradeLabel: 'Groep, year or age' };
const marketOf = page => Object.assign({}, NL_MARKET, page.market || {});

// Opt-in (market.stripTrunk): a visitor who types the country code again, or the
// national trunk 0 (UK 07700 900123), would otherwise send +44 07700..., which
// is not a dialable number. Markets without the flag render exactly as before.
function trunkFix(m, v) {
  if (!m.stripTrunk) return '';
  const cc = String(m.dial).replace(/\D/g, '');
  return `if(${v}.indexOf('00${cc}')===0)${v}=${v}.slice(${cc.length + 2});else if(${v}.indexOf('${cc}')===0&&${v}.length>${m.minDigits || 10})${v}=${v}.slice(${cc.length});if(${v}.charAt(0)==='0')${v}=${v}.slice(1);`;
}
...
${page.personalityCss.trim()}
  ...
  <meta property="og:locale" content="${mk.locale}">
  ...
  <meta name="geo.region" content="${mk.geoRegion}">
