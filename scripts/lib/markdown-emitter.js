 ...
 *
 * Produces clean markdown twins of generated HTML pages so AI agents
 * (Claude Code, Cursor, OpenCode, etc.) get a token-efficient version
 * of every page when they fetch the .md companion.
 *
 * Public API:
 *   - blogToMarkdown(blogData)
 *   - courseToMarkdown(courseData)
 *   - chapterNotesToMarkdown(chapterData, langMeta)
 *   - chapterPracticeToMarkdown(chapterData, langMeta)
 *   - htmlToMarkdown(html)
...
// ---------- Course (generic recursive walker) ----------
...
function renderObject(obj, depth) {
    ...
    if (obj.title) out += `${'#'.repeat(lvl)} ${stripInlineHtml(obj.title)}\n\n`;
