---
title: "Introduction to Python - What It Is, Why Learn It, and Where It Is Used | Modern Age Coders"
description: "Learn what Python is, why it is the most popular programming language for beginners, and where it is used in the real world. A complete introduction for students aged 10-16."
slug: introduction-to-python
canonical: https://learn.modernagecoders.com/resources/python/introduction-to-python/
category: "Python"
keywords: ["what is python", "python introduction for beginners", "why learn python", "python programming language", "python for kids", "python for students", "introduction to python programming", "python uses"]
---
...
1. **You write your code** in a text file and save it with a `.py` extension (for example, `hello.py`).
2. **You run the file** by typing `python hello.py` in your terminal or command prompt.
3. **The Python interpreter** reads your file from top to bottom, executing each line one at a time.
4. **If there is an error**, the interpreter stops at that line and shows you an error message telling you what went wrong and on which line.
...
The `print()` function displays text on the screen. Whatever you put inside the parentheses and quotes will be shown as output. This is the simplest and most famous first program in any language.
...
Python can perform mathematical operations just like a calculator. The `+` operator adds, `*` multiplies, `-` subtracts, `/` divides, and `**` raises to a power (2 to the power of 10 is 1024).
...
### Comparing Hello World: Python vs C++ vs Java
...
This comparison shows why Python is considered beginner-friendly. To print a simple message, Python needs just 1 line, while C++ needs about 6 lines and Java needs about 5 lines. Python removes all the unnecessary setup code so you can focus on the task at hand.
...
In the Python REPL (interactive shell), you can type expressions directly after the `>>>` prompt and see results immediately. Here we calculate: the sum of 15 and 27, the number of hours in a year (365 days times 24 hours), 100 divided by 3, and 2 raised to the power of 8. Notice that division always gives a decimal result in Python 3.
...
### A Multi-Line Program with Multiple Print Statements
...
I am going to build amazing things!
```

### Using Comments to Explain Your Code

```python
# This program greets a student
...
Even with just the `print()` function, you can create visual patterns. This program prints a rectangle made of asterisk (*) characters. Each `print()` call outputs one row. Later in the course, you will learn to use loops to create such patterns more efficiently.
...
In Python 3, `print` is a function, and all functions require parentheses around their arguments. The syntax `print "Hello"` was valid in Python 2, but Python 2 is no longer supported. Always use `print("Hello")` with parentheses in Python 3. This is the most common mistake that beginners make when following old tutorials written for Python 2.

### Using the Wrong Case (Print vs print)
...
Python is a **case-sensitive** language. This means `print`, `Print`, and `PRINT` are all treated as completely different names. The built-in function is `print` (all lowercase). Writing `Print` with a capital P makes Python think you are referring to something else that does not exist, resulting in a NameError.
...
The file may not be recognized as a Python script by your editor or operating system.
...
Python source code files must be saved with the `.py` extension (for example, `hello.py`, `calculator.py`). While Python can technically run a file with any extension if you specify it directly, saving with `.py` is essential because: (1) your code editor uses the extension to provide syntax highlighting and helpful features, (2) your operating system needs it to know which program should open the file, and (3) it is the standard convention that all Python developers follow.

### Using the Wrong Python Version Command
...
# Typing in terminal:
python hello.py
# But getting Python 2 instead of Python 3
```

On some systems (especially Linux and Mac), the command 'python' may open Python 2, which is outdated.
...
Some operating systems have both Python 2 and Python 3 installed. On these systems, typing `python` might open Python 2, while `python3` opens Python 3. Always check which version you are using by running `python --version` or `python3 --version`. You should see a version number starting with 3 (like 3.12.0). If you are on Windows and installed Python recently, `python` usually points to Python 3.
...
In Python, a string (text) must start and end with the same type of quote. If you start with a double quote (`"`), you must end with a double quote. If you start with a single quote (`'`), you must end with a single quote. Mixing them up causes a SyntaxError because Python cannot find the matching closing quote.
...
*Source: https://learn.modernagecoders.com/resources/python/introduction-to-python/*
*Practice questions: https://learn.modernagecoders.com/resources/python/introduction-to-python/practice/*
