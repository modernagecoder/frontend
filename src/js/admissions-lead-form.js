/* admissions-lead-form.js
   Handler for the Admissions Projects cluster forms (parent / student enquiries).
   Posts to the SAME backend the site's contact form uses, so these leads land in the
   owner's existing inbox alongside everything else.

   Form markup contract:
     <form id="leadForm"
           data-service="Portfolio Program"
           data-src="passion-project-ideas-computer-science"> ... </form>
   Fields (by name): name, email, phone, year, country, deadline, message
*/
(function () {
  'use strict';
  ...
  form.addEventListener('submit', function (ev) {
    ...
    if (message.length < 10) { setErr('message', 'Tell us a little about the project or goal (10+ characters)'); ok = false; }
    if (!ok) return;

    var service = form.getAttribute('data-service') || 'Portfolio Program';
    var src = form.getAttribute('data-src') || 'admissions-projects';

    // The backend requires the `contact` field to be exactly 10 digits. That rule is
    // India shaped and these pages are aimed at the US, UK, Singapore and Canada, so
    // we never let it drop a lead: the REAL phone the visitor typed always travels in
    // the message body (which is not validated), and `contact` carries a best-effort
    // 10-digit form.
    ...
    // Fold the admissions context into the message so it lands with the lead.
    var composed =
      '[ADMISSIONS PROJECT] Interest: ' + service + '\n' +
      'Student year or grade: ' + (year || 'Not provided') + '\n' +
      'Country / target market: ' + (country || 'Not provided') + '\n' +
      'Application deadline: ' + (deadline || 'Not provided') + '\n' +
      ...
      .then(function (r) {
        if (r.ok) {
          if (typeof fbq === 'function') {
            fbq('track', 'Lead', { content_name: service, content_category: 'Admissions Projects' });
          }
